...
import src.util.PaginationHelper;
...
public class BidController implements Serializable {
    ...
    @EJB
    private src.BidFacade ejbFacade;
    ...
    public Bid getSelected() {
        if (current == null) {
            current = new Bid();
            current.setBidPK(new src.BidPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public static class BidControllerConverter implements Converter {
        ...
        String getStringKey(src.BidPK value) {
